"""
=============================================================================
Template: AES-CBC Encryption + SHA-256 Hashing + ElGamal Signature + RBAC
=============================================================================
PURPOSE:
This program simulates a secure Role-Based Access Control (RBAC) system.

ALGORITHMS & WHY:
- AES-CBC: Symmetric encryption for strong data confidentiality (16-byte block size).
...
# CUSTOMIZE: AES Key and IV
# Must be exactly 16, 24, or 32 bytes for AES
CIPHER_KEY = b"1234567890123456" # 16 bytes
...
def encrypt_aes_cbc(data_str: str) -> bytes:
    """
    ...
    """
    # Create a new AES cipher instance using the key and IV
    cipher = AES.new(CIPHER_KEY, AES.MODE_CBC, IV)
    # Pad the encoded string to the AES block size and encrypt
    return cipher.encrypt(pad(data_str.encode(), AES.block_size))

def decrypt_aes_cbc(ciphertext: bytes) -> str:
    """
    Decrypts ciphertext bytes using AES in CBC mode.
    Parameters:
...
def role1_action(username: str):
    ...
    encrypted_data = encrypt_aes_cbc(data)
...
def role2_action():
    ...
    if integrity_pass and sig_pass:
        ...
        try:
            print(decrypt_aes_cbc(enc_data))
