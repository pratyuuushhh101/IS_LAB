"""
==============================================================================
Role-Based Crypto System Template (DES + SHA256 + RSA Signature)
Purpose: Ready-to-run template for exam questions involving RBAC and DES.

Algorithms Used:
- DES-CBC: For data encryption.
...
# CUSTOMIZE: Global DES Key and IV for the system
CIPHER_KEY = b"12345678" # 8 bytes for DES
IV = b"12345678"

# Global dictionary for user RSA keys for signatures
USER_KEYS = {}
for uname in USERS:
...
def des_encrypt(data_bytes):
    """
    Encrypts data using DES in CBC mode.
    Parameters: data_bytes (bytes)
    Returns: bytes (ciphertext)
    """
    # CUSTOMIZE: DES-CBC Encryption
    # Create DES cipher in CBC mode with the key and IV
    cipher = DES.new(CIPHER_KEY, DES.MODE_CBC, IV)
    # Pad data to DES block size (8 bytes) and encrypt
    return cipher.encrypt(pad(data_bytes, DES.block_size))

def des_decrypt(cipher_bytes):
    """
    Decrypts DES-CBC ciphertext.
    Parameters: cipher_bytes (bytes)
    Returns: bytes (plaintext)
    """
    # CUSTOMIZE: DES-CBC Decryption
    # Recreate DES cipher for decryption
    cipher = DES.new(CIPHER_KEY, DES.MODE_CBC, IV)
    # Decrypt and unpad to recover original data
    return unpad(cipher.decrypt(cipher_bytes), DES.block_size)
...
def uploader_action(username):
    ...
    enc_data = des_encrypt(data_bytes)
